...
int vdread(int fd, char *buffer, int bytes)
{
	...
	if (openfiles[fd].inuse == 0)
		return (-1);
	// Checar si el sector de boot está en memoria, si no lo esta cargarlo a memoria
	if (!sb_in_mem)
	{
		result = vdreadseclog(0, 0, (char *)&sb);
		sb_in_mem = 1;
	...
	while (cont < bytes)
	{
		// Obtener la dirección del bloque que corresponde a la posicion actual
		currptr = currpostoptr(fd);

		if (currptr == NULL) // Si no existe
			return -1;

		currblock = *currptr; //Numero de bloque

		if (currblock == 0)//Si no tiene nada
		{
			currblock = nextfreeblock();// Busca uno libre

			*currptr = currblock;//Apunta al actual
			assignblock(currblock);//Y lo asigna
			sector = currinode;//Escribe el sector de la tabla de nodoi
			result = vdreadseclog(0,inicio_nodos_i+sector, (char *)&inode[sector*8]); //Lee el sector logico
		}
		//si no se encuentra en memoria lee el bloque 
		if (openfiles[fd].currbloqueenmemoria != currblock) 
		{												
			readblock(currblock, openfiles[fd].buffer); // Lee el buffer del bloque actual de los archivos abiertos
			openfiles[fd].currbloqueenmemoria = currblock; 
		}
		buffer[cont] = openfiles[fd].buffer[openfiles[fd].currpos % 1024]; //Copia el buffer
		openfiles[fd].currpos++; //Aumenta la psicion
		if (openfiles[fd].currpos > inode[currinode].size)
		{
			return cont;
		}
